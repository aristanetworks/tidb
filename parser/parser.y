...
%token	<ident>
	...
	stddevPopCH           "STDDEVPOP"
	stddevSamp            "STDDEV_SAMP"
	stddevSampCH          "STDDEVSAMP"
	stop                  "STOP"
	strict                "STRICT"
	strong                "STRONG"
	subDate               "SUBDATE"
...
|	builtinAny '(' BuggyDefaultFalseDistinctOpt Expression ')' OptWindowingClause
	{
		if $6 != nil {
			$$ = &ast.WindowFuncExpr{F: $1, Args: []ast.ExprNode{$4}, Distinct: $3.(bool), Spec: *($6.(*ast.WindowSpec))}
		} else {
...
|	"QUANTILE" '(' BuggyDefaultFalseDistinctOpt Expression ',' NumLiteral ')' OptWindowingClause
	{
		if $8 != nil {
			$$ = &ast.WindowFuncExpr{F: $1, Args: []ast.ExprNode{$4, ast.NewValueExpr($6, "", "")}, Distinct: $3.(bool), Spec: *($8.(*ast.WindowSpec))}
		} else {
			$$ = &ast.AggregateFuncExpr{F: $1, Args: []ast.ExprNode{$4, ast.NewValueExpr($6, "", "")}, Distinct: $3.(bool)}
		}
	}
...
|	"LEADINFRAME" '(' Expression OptLeadLagInfo ')' OptNullTreatment WindowingClause
	{
		args := []ast.ExprNode{$3}
		if $4 != nil {
			args = append(args, $4.([]ast.ExprNode)...)
...
|	"LAGINFRAME" '(' Expression OptLeadLagInfo ')' OptNullTreatment WindowingClause
	{
		args := []ast.ExprNode{$3}
		if $4 != nil {
			args = append(args, $4.([]ast.ExprNode)...)
