...
func TestBuiltin(t *testing.T) {
	table := []testCase{
		...
		{`select stddevpop(c1), stddevpop(all c1), stddevpop(distinct c1) from t`, true, "SELECT STDDEV_POP(`c1`),STDDEV_POP(`c1`),STDDEV_POP(DISTINCT `c1`) FROM `t`"},
		{`select stddevpop(c1, c2) from t`, false, ""},
		...
		{`select stddevsamp(c1), stddevsamp(all c1), stddevsamp(distinct c1) from t`, true, "SELECT STDDEVSAMP(`c1`),STDDEVSAMP(`c1`),STDDEVSAMP(DISTINCT `c1`) FROM `t`"},
		{`select stddevsamp(c1, c2) from t`, false, ""},
		...
		{`select quantile(c1, 0.5) from t;`, true, "SELECT QUANTILE(`c1`, 0.5) FROM `t`"},
		{`select quantile(distinct c1, 0.5) from t;`, true, "SELECT QUANTILE(DISTINCT `c1`, 0.5) FROM `t`"},
		{`select quantile(distinctrow c1, 0.5) from t;`, true, "SELECT QUANTILE(DISTINCT `c1`, 0.5) FROM `t`"},
		{`select quantile(distinct all c1, 0.5) from t;`, true, "SELECT QUANTILE(DISTINCT `c1`, 0.5) FROM `t`"},
		{`select quantile(distinctrow all c1, 0.5) from t;`, true, "SELECT QUANTILE(DISTINCT `c1`, 0.5) FROM `t`"},
		{`select quantile(c1,c2) from t;`, false, ""},
		{`select quantile(c2) from t;`, false, ""},
		{`select median(c1) from t;`, true, "SELECT MEDIAN(`c1`) FROM `t`"},
		{`select median(distinct c1) from t;`, true, "SELECT MEDIAN(DISTINCT `c1`) FROM `t`"},
		{`select median(distinctrow c1) from t;`, true, "SELECT MEDIAN(DISTINCT `c1`) FROM `t`"},
		{`select median(distinct all c1) from t;`, true, "SELECT MEDIAN(DISTINCT `c1`) FROM `t`"},
...
func TestWindowFunctions(t *testing.T) {
	table := []testCase{
		...
		{`SELECT LAGINFRAME(val) OVER (w) FROM t;`, true, "SELECT LAGINFRAME(`val`) OVER (`w`) FROM `t`"},
		{`SELECT LAGINFRAME(val, 1) OVER w FROM t;`, true, "SELECT LAGINFRAME(`val`, 1) OVER `w` FROM `t`"},
		{`SELECT LAGINFRAME(val, 1) OVER (w) FROM t;`, true, "SELECT LAGINFRAME(`val`, 1) OVER (`w`) FROM `t`"},
		{`SELECT LAGINFRAME(val1+val2) OVER (w) FROM t;`, true, "SELECT LAGINFRAME(`val1`+`val2`) OVER (`w`) FROM `t`"},
		{`SELECT LAGINFRAME(val, 1, 0) OVER (w) FROM t;`, true, "SELECT LAGINFRAME(`val`, 1, 0) OVER (`w`) FROM `t`"},
		...
		{`SELECT LEADINFRAME(val) OVER w FROM t;`, true, "SELECT LEADINFRAME(`val`) OVER `w` FROM `t`"},
		{`SELECT LEADINFRAME(val, 1) OVER w FROM t;`, true, "SELECT LEADINFRAME(`val`, 1) OVER `w` FROM `t`"},
		{`SELECT LEADINFRAME(val, 1) OVER (w) FROM t;`, true, "SELECT LEADINFRAME(`val`, 1) OVER (`w`) FROM `t`"},
		{`SELECT LEADINFRAME(val1+val2) OVER w FROM t;`, true, "SELECT LEADINFRAME(`val1`+`val2`) OVER `w` FROM `t`"},
		{`SELECT LEADINFRAME(val, 1, 0) OVER (w) FROM t;`, true, "SELECT LEADINFRAME(`val`, 1, 0) OVER (`w`) FROM `t`"},
