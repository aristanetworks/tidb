...
// LambdaExpr is for lambda expression
type LambdaExpr struct {
	exprNode
	Param *LambdaParamExpr
	Expr  ExprNode
}

// Restore implements Node interface.
func (n *LambdaExpr) Restore(ctx *format.RestoreCtx) error {
	n.Param.Restore(ctx)
	ctx.WritePlain("->")
	n.Expr.Restore(ctx)
	return nil
...
func (n *LambdaExpr) Format(w io.Writer) {
	panic("Not implemented")
}

// Accept implements Node Accept interface.
func (n *LambdaExpr) Accept(v Visitor) (Node, bool) {
	newNode, skipChildren := v.Enter(n)
	if skipChildren {
		return v.Leave(newNode)
	}
	n = newNode.(*LambdaExpr)

	newParam, ok := n.Param.Accept(v)
	if !ok {
		return n, false
	}
	n.Param = newParam.(*LambdaParamExpr)

	newExpr, ok := n.Expr.Accept(v)
	if !ok {
		return n, false
	}
	n.Expr = newExpr.(ExprNode)

	return v.Leave(n)
}

// LambdaParamExpr is for parameter of lambda expression
type LambdaParamExpr struct {
	exprNode
	Name model.CIStr
}

// Restore implements Node interface.
func (n *LambdaParamExpr) Restore(ctx *format.RestoreCtx) error {
	ctx.WritePlain(n.Name.String())
	return nil
}

// Format the ExprNode into a Writer.
func (n *LambdaParamExpr) Format(w io.Writer) {
	panic("Not implemented")
}

// Accept implements Node Accept interface.
func (n *LambdaParamExpr) Accept(v Visitor) (Node, bool) {
	newNode, skipChildren := v.Enter(n)
	if skipChildren {
		return v.Leave(newNode)
	}
	n = newNode.(*LambdaParamExpr)
	return v.Leave(n)
}
